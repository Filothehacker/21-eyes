...
def convert_annotation(xml_fn, txt_dir):
    ...
    if not os.path.exists(txt_dir):
        os.makedirs(txt_dir)
    txt_fn = os.path.join(txt_dir, base + ".txt")
    ...
    for obj in root.iter('object'):
        ...
        bb = convert((w,h), b)
        out_file.write(f"{cls_id} {bb[0]:0.16f} {bb[1]:0.16f} {bb[2]:0.16f} {bb[3]:0.16f}\n")
    in_file.close()
    out_file.close()
...
list_fn=sys.argv[3]
txt_dir = os.path.join(os.path.dirname(images_dir), 'txt_annotations')
...
for i,xml_fn in enumerate(glob(images_dir+"/*.xml")):
    ...
    convert_annotation(xml_fn, txt_dir)
