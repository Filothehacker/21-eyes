...
from utils_yolo import train, eval
import wandb
import yaml
...
if __name__ == "__main__":
    ...
    model_config_path = os.path.join(cwd, "src", "yolov5_ultralytics", "models", "yolov5s.yaml")
    yolo_v5 = Model(model_config_path, ch=3, nc=len(CLASSES))

    # Load the pre-trained weights for the convolution
    checkpoint_path = os.path.join(cwd, "models", "yolov5s.pt")
    checkpoint = torch.load(checkpoint_path, map_location=DEVICE)
    model_state = checkpoint['model'].state_dict()
    filtered_state = {
        k: v for k, v in model_state.items()
        if k in yolo_v5.state_dict() and v.shape == yolo_v5.state_dict()[k].shape
    }
    yolo_v5.load_state_dict(filtered_state, strict=False)

    # Set the hyperparameters
    hyperparameters_path = os.path.join(cwd, "src", "yolov5_ultralytics", "data", "hyps", "hyp.scratch-low.yaml")
    with open(hyperparameters_path, "r") as f:
        yolo_v5.hyp = yaml.safe_load(f)
    
    # Create all necessary components for the training
    criterion = ComputeLoss(yolo_v5)

    optimizer = torch.optim.Adam(
        params=yolo_v5.parameters(),
        lr=train_config["lr"],
    ...
    model_architecture = str(yolo_v5)
    archive_path = os.path.join(cwd, "models", "yolo_v5_arch.txt")
    with open(archive_path, "w") as archive_file:
        archive_file.write(model_architecture)
    artifact = wandb.Artifact("yolo_v5_arch", type="model")
