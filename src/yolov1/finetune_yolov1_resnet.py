...
import wandb
import yaml
from yolov1_resnet50 import YoloV1
...
def train_loop(model, train_loader, development_loader, criterion, optimizer, scheduler, scaler, device, num_epochs, cwd):
    ...
    for epoch in range(num_epochs):
        ...
        if eval_map > best_eval_map:
            ...
            checkpoint_path = os.path.join(cwd, "models", "yolov1_resnet50.pth")
            torch.save(
                {
...
if __name__ == "__main__":
    ...
    model_config_path = os.path.join(cwd, "configurations", "yolov1_resnet50.json")
    ...
    archive_path = os.path.join(cwd, "models", "yolov1_resnet50_arch.txt")
    with open(archive_path, "w") as archive_file:
        archive_file.write(model_architecture)
    artifact = wandb.Artifact("yolov1_resnet50_arch", type="model")
